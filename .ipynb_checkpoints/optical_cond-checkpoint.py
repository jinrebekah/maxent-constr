...
class sigma:
    ...
    def calc_sigma_xy(self):
        ...
        if self.bs:
            ...
            #     bs_list.append(bs_dict)
            ##### this is dumb but for now, switch to a while loop for the occasional failed bootstrap
            i = 0 # keeps track of successful attempts
            max_attempts = int(self.bs*1.5)
            attempts = 0
            with tqdm(total=self.bs, desc='Sigma_xy bootstraps') as pbar:
                while i < self.bs:
                    if attempts > max_attempts:
                        raise RuntimeError("Too many failed bootstrap attempts")
...
def calc_sig_xy_0(sig):
    ...
    ### temporary alternative test: is something going wrong with my kramer's kronig? try just obtaining D.C. value directly from im...
    im_sig_xy_bs = np.array(sig.results['im_sig_xy'].tolist())
    ws = sig.ws
    integral = np.trapz(im_sig_xy_bs/ws, x=ws, axis=1)/np.pi
    
    print("Comparing DC sig values:")
    print(sig_xy_0, np.mean(integral))
...
def plot_results(sig, sig_names=None, bs_idx=None, bs_mode='errorbar', xlim=None):
    ...
    if xlim is not None:
        if num_plots==1:
            ...
            for i in range(num_plots): ax[i].set_xlim(xlim)
...
def plot_sigma(sig, ax, sigma_name, bs_idx=None, bs_mode='errorbar', color='#0C5DA5', label='', show_opt_info=True):
    ...
    if sig.settings_xy['xx_scale'] != 1:
        sigma_name_dict['sig_sum'] = str(sig.settings_xy['xx_scale']) + r'(Re[$\sigma_{xx}(\omega)$]) + Im[$\sigma_{xy}(\omega)$]'
...
def get_sig_pickle(path, nflux=None, n=None, U=None, beta=None, mu=None):
    ...
    if pickle_path is None:
        raise FileNotFoundError(f"No pickle found for nflux={nflux}, n={n}, U={U}, beta={beta}, mu={mu} in {path}")
