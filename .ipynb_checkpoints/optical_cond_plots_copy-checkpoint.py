...
######################## plots vs. B
def plot_vs_B(df, mode, nfluxes, U, n, betas,
              Nx=None, Ny=None, tp=None, B0=0,
              ax=None, fmt='-', show_legend=True):
    
    ### mode options
    LABELS = {
        'sig_xx': (r'$\sigma_{xx}$', 'sig_xx'),
        'sig_xy': (r'$\sigma_{xy}$', 'sig_xy'),
        'sig_xx_proxy': (r'$\sigma_{xx}^1$', 'sig_xx_proxy1'),
        'rho': (r'$\rho_{xx}(B)$', 'rho_xx'),
        'rho_proxy1': (r'$\rho_1$', 'rho_proxy1'),
        'rho_proxy2': (r'$\rho_2$', 'rho_proxy2'),
    }

    y_label, key = LABELS[mode]
    ...
    # ---- setup ----
    if ax is None:
        fig, ax = plt.subplots(figsize=(default_figsize[0], default_figsize[1]*1.5))

    colors = sns.color_palette('husl', len(betas))

    # ---- loop ----
    for k, beta in enumerate(betas):
        df_subset = _get_subset(df, Nx, Ny, tp, U, n, beta, nfluxes)
        if df_subset.empty:
            continue

        x = df_subset['nflux']/(Nx*Ny)

        # ---- mode-specific logic ----
        if mode == 'rho_diff':
            rho0 = df_subset[df_subset['nflux']==B0]['rho_xx'].item()
            rho0_err = df_subset[df_subset['nflux']==B0]['rho_xx_err'].item()
            y = df_subset['rho_xx'] - rho0
            yerr = np.sqrt(df_subset['rho_xx_err']**2 + rho0_err**2)
        else:
            y = df_subset[key]
            yerr = df_subset[key + '_err']

        _plot_err(ax, x, y, yerr,
                  label=rf'$\beta$={beta}',
                  color=colors[k],
                  fmt=fmt)
    ...
    ax.set_xlabel(r'$B$ $[\Phi_0/a^2]$', fontsize=12)
    ax.set_ylabel(y_label, fontsize=12)

    if show_legend:
        ax.legend(labelspacing=0.35, fontsize=9)
...
def plot_vs_B(ax=ax, df, nfluxes, n, U, betas, y_name, y_err_name, y_label, Nx=None, Ny=None, tp=None, show_legend=True):
    # y_name is the name of df column to plot
    # y_label is the text for y-axis label
    ...
    for k, beta in enumerate(betas):
        ...
        if df_subset.empty:
            continue
        ys = df_subset[y_name]
        y_errs = df_subset[y_err_name]
        Bs_plot = df_subset['nflux']/(Nx*Ny)
        ax.errorbar(Bs_plot, ys, yerr=y_errs, label=rf'$\beta$={beta}', fmt=fmt, markersize=2, color = colors[k], lw=lw, elinewidth=1, capsize=0.8)
    ...
    # for k, beta in enumerate(betas):
    #     ax.errorbar(Bs_plot, sigs, yerr=sig_errs, label=rf'$\beta$={beta}', fmt=fmt, markersize=2, color = colors[k], lw=lw, elinewidth=1, capsize=0.8)

    plt.tight_layout()
        
        
def plot_dc_sig_B(df, nfluxes, n, U, betas, Nx=None, Ny=None, tp=None, mode='xx', ylim=None, xlim=None, ax=None, fmt='-', show_legend=True):
    """Plots sigma(w=0) vs. B. Choose between 'xx', 'xy', 'proxy_xx', or proxy_xy."""
    if mode=='xx':
        sig_label = 'sig_xx'
        ...
        sig_label = 'sig_xx_proxy1'
        y_label = rf'$\sigma_{{xx}}^1$'
        # for now, just invert 1st rho_xx proxy when plotting lol
    elif mode=='proxy_xy':
        sig_label = 'sig_xy_proxy'
        y_label = rf'$\sigma_{{xy}}^1$'
    else:
        pass
    ...
    plot_vs_B(df, nfluxes, ns, Us, betas, ylabel=ylabel, Nx=Nx, Ny=Ny, ax=ax, show_legend=show_legend)

def plot_rho_B(df, nfluxes, ns, Us, betas,  Nx=None, Ny=None, tp=None, mode='rho', B0=0, ylims=None, xlim=None, suptitle='', ax=None, fmt='-', show_legend=True):
    ...
    ax.set_xlabel(r'$B$ $[\Phi_0/a^2]$', fontsize=12)
    ax.set_ylabel(y_label, fontsize=12)
    if show_legend: ax.legend(labelspacing = 0.35, fontsize=9)
...
######################## plots vs. T
def plot_rho_T(df, nfluxes, ns, Us, Nx=None, Ny=None, tp=None, ylim=None, xlim=None, fmt='-', ax=None, show_legend=True, colors=None, labels=None, show_title= True):
    Nx = _resolve_param(df, Nx, "Nx")
    Ny = _resolve_param(df, Ny, "Ny")
    tp = _resolve_param(df, tp, "tp")

    if colors is None:
        colors = sns.color_palette('tab10', len(nfluxes))
    ...
    for i, U in enumerate(Us):
        for j, n in enumerate(ns):
            for k, nflux in enumerate(nfluxes):
                df_subset = df[(df['Nx']==Nx) & (df['Ny']==Ny) & (df['tp']==tp) & (df['U']==U) & (df['nflux']==nflux) & (df['n']==n)].sort_values(by='beta', ascending=False)
                if df_subset.empty:
                    continue
                ...
                Ts = 1/betas
                lw = 1 if ':' in fmt else 0.75
                ax[i, j].errorbar(Ts, rhos, yerr=rho_errs, label=labels[k] if labels is not None else rf'$B$={nflux}/{Nx*Ny}', fmt=fmt, markersize=2, color = colors[list(nfluxes).index(nflux)], lw=lw, elinewidth=1, capsize=0.8)
    ...
    for j, n in enumerate(ns):
        ...
        if show_title: ax[0, j].set_title(f'n = {n}', fontsize=14)
    for i, U in enumerate(Us):
        ax[i, 0].set_ylabel(r'$\rho$ [$\hbar/e^2$]', fontsize=12)
        # if show_title: ax[i, -1].annotate(f'U = {U}', (1.02, 0.48), rotation=270, xycoords='axes fraction', fontsize=14)
    if show_legend: ax[0,-1].legend(labelspacing = 0.35, fontsize=9)
    ...
    for i, U in enumerate(Us):
        for j, n in enumerate(ns):
            for k, nflux in enumerate(nfluxes):
                ...
                lw = 1 if ':' in fmt else 0.75
                ax[i, j].errorbar(Ts_plot, sigs, yerr=sig_errs, label=labels[k] if labels is not None else rf'$B$={nflux}/{Nx*Ny}', fmt=fmt, markersize=2, color = colors[list(nfluxes).index(nflux)], lw=lw, elinewidth=1, capsize=0.8)
...
def plot_doubleocc_T(df, nfluxes, ns, Us, betas,  Nx=None, Ny=None, tp=None, B0=0, ylims=None, xlim=None, suptitle='', ax=None, fmt='-', show_legend=True, colors=None, labels=None):
    Nx = _resolve_param(df, Nx, "Nx")
    Ny = _resolve_param(df, Ny, "Ny")
    tp = _resolve_param(df, tp, "tp")
    ...
    for i, U in enumerate(Us):
        for j, n in enumerate(ns):
            for k, nflux in enumerate(nfluxes):
                ...
                Ts_plot = 1/betas_plot
                lw = 1 if ':' in fmt else 0.75
                ax[i, j].errorbar(Ts_plot, double_occs, yerr=double_occ_errs, label=labels[k] if labels is not None else rf'$B$={nflux}/{Nx*Ny}', fmt=fmt, markersize=2, color = colors[list(nfluxes).index(nflux)], lw=lw, elinewidth=1, capsize=0.8)
    ...
    y_label = r'$\langle n_uparrow n_ \rangle$'
    for j, n in enumerate(ns):
        ax[-1, j].set_xlabel('T/t', fontsize=12)
        # ax[0, j].set_title(f'n = {n}', fontsize=14)
    for i, U in enumerate(Us):
...
def plot_kohler2(df, nfluxes, ns, Us, betas, Nx=None, Ny=None, tp=None, calc_fit=False, ax=None, suptitle='', xlim=None, ylim=None, show_legend=True, fmt='-', alpha=1):
    Nx = _resolve_param(df, Nx, "Nx")
    Ny = _resolve_param(df, Ny, "Ny")
    tp = _resolve_param(df, tp, "tp")
    
    # this is just loglog vs B/rho_0 lmfao kms
    colors = sns.color_palette('husl', len(betas))
    if len(Us)==1 and len(ns)==1:
        figsize = (default_figsize[0], default_figsize[1]*1.5)
    else:
    ...
    for i, U in enumerate(Us):
        # pad = 0.15
        # min = np.min(df[(df['U']==U)]['MR'])
        # max = np.max(df[(df['U']==U)]['MR'])
        # ylim = (min-(max-min)*pad, max+(max-min)*pad)
        
        for j, n in enumerate(ns):
            for k, beta in enumerate(betas):
                # Get subset of df for specific U, beta, n
                # df_subset = df[(df['U']==U) & (df['beta']==beta) & (df['n']==n) & (df['nflux'].isin(nfluxes))].sort_values(by='nflux')c
                df_subset = df[(df['Nx']==Nx) & (df['Ny']==Ny) & (df['tp']==tp) & (df['U']==U) & (df['beta']==beta) & (df['n']==n)].sort_values(by='nflux').reset_index()
                # (df['nflux'].isin(nfluxes))
                if df_subset.empty:
                    continue
                # print(df_subset)
                # nfluxes_plot = df_subset['nflux']
                ...
                ax[i, j].errorbar(Bs_plot/rho_B0, MRs_plot, yerr=MR_errs_plot, xerr=B_errs, label=rf'$\beta$={beta}', fmt=fmt, markersize=2, color = colors[k], capsize=0.8, lw=0.75, elinewidth=1, alpha=alpha)
                # ax[i, j].set_yscale('log', nonpositive='clip')
                # ax[i, j].set_xscale('log', nonpositive='clip')
                if ylim is not None:
                    ax[i, j].set_ylim(*ylim)
                ...
                if calc_fit:
                    ...
                    print(f"{n:>6}  {U:>4}  {beta:>4}  {np.exp(params[0]):10.2f} ± {np.exp(params[0])*perr[0]:<10.2f}  "
                    f"{params[1]:10.3f} ± {perr[1]:<10.3f}")
            # if i!=0: ax[i, j].tick_params(labelleft=False)
            
            # ax[j, i].annotate(f'n={n}', xy=(0.72, 0.05), xycoords='axes fraction', fontsize=12)
            # ax[i, j].annotate(f'n={n}', xy=(0.06, 0.91), xycoords='axes fraction', fontsize=12)
            ax[i, j].tick_params(axis='both', which='major', labelsize=12)
            # ax[i, j].set_xlim(1e-2, 1)
            # import matplotlib.ticker as ticker

            # ax[i, j].xaxis.set_major_formatter(ticker.LogFormatterMathtext())
            ...
            if not (ax[i, j].lines or ax[i, j].collections):
                ax[i, j].set_facecolor('gainsboro')
    for j, n in enumerate(ns):
        ax[-1, j].set_xlabel(r'$B/\rho_0$', fontsize=12)
        # ax[0, j].set_title(f'n = {n}', fontsize=14)
    for i, U in enumerate(Us):
        ax[i, 0].set_ylabel('MR [%]', fontsize=12)
        # ax[i, -1].annotate(f'U = {U}', (1.02, 0.48), rotation=270, xycoords='axes fraction', fontsize=14)
    ...
    if show_legend:
        ax[0,-1].legend(labelspacing = 0.4, fontsize=10)
    # plt.tight_layout()
